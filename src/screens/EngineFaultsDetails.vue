<template>
    <v-container fluid fill-height grid-list-md style="padding:0px 10px 5px 10px;height:100%;width:100%;background-color:white;">
        <v-layout v-if="fault!=undefined" column fill-height>
            ...
            <v-flex ref="tlChart" v-for="n in 6" :key="n" d-flex md4 style="height: 24rem;">
                <TimelineChart v-if="active && chartData[params[n-1]].series[0].data.length" v-bind:active="active" v-bind:chartOptions="chartData[params[n-1]]" v-bind:title="titles[params[n-1]]" class="fault-charts"/>
            </v-flex>
        </v-layout>
        <div v-else style="width:100%;height:100%;background-color:white;display:flex;justify-content:center;align-items:center;">
            <v-img :src="require('../assets/check-mark-circle3.png')" contain height="200"></v-img>
        </div>
    </v-container>
</template>
                ...
                if (typeof this.fault != 'undefined') {
                    ...
                    for (let p = 0; p < mapParams.length; p++) {
                        if ((mapParams[p] == 'tExhC') || (mapParams[p] == 'pcomp') || (mapParams[p] == 'pmax'))
                            ...
                            this.$set(this.titles, this.params[p], globalStore.mapping[mapParams[p]]);
                    }
                }
            },
            setValuesData() {
                ...
                for (let p = 0; p < this.params.length; p++) {
                    ...
                    for (let i = 0; i < timeStamps.length; i += 3) {
                        ...
                        if (elCount[p] == 0) {
                            val = globalStore.signals[timeStamps[i]][this.params[p]];

                            if (typeof val == 'undefined') {
                                val = globalStore.signals[timeStamps[i]][this.params[p].replace(/\d+$/, '')];
                            }
                        } else {
                            val = this.average(this.params[p], elCount[p], globalStore.signals, timeStamps[i]);
                        }

                        if (isNaN(val))
                            val = -1000;

                        if (this.params[p] == 'fRailPres') {
                            ...
                            if (elCount[p] == 0)
                                ...
                                rfr = this.average(refs[p], elCount[p], globalStore.reference, timeStamps[i]);
                        } else
                            rfr = -1000;
                        ...
                        if (val != -1000 && rfr != -1000 && Object.keys(globalStore.limits).includes(mapParams[p])) {
                            let minLimit = globalStore.limits[mapParams[p]][0],
                                maxLimit = globalStore.limits[mapParams[p]][1];
