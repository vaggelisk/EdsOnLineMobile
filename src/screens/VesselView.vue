            ...
            <v-flex>
                <v-layout>
                    <v-flex>
                        <v-layout column>
                            <v-flex>
                                <EngineState v-bind:data="engineData"/>
                            </v-flex>

                            <v-flex>
                                <SubsystemState v-bind:data="subSystemData"/>
                            </v-flex>

                            <v-flex>
                                <PerformanceWidget v-bind:cardData="cardData" v-bind:gaugeData="gaugeData"/>
                            </v-flex>

                            <v-flex>
                                <v-card fill-height style="height:100%;width:100%;">
                                    <v-card-actions fill-height style="height:100%;padding-top:10px;">
                                        <v-layout column fill-height>
                                            <v-flex style="padding:0px;">
                                                <Gauge v-bind:data="gaugeData['ensp']"/>
                                            </v-flex>
                                            <v-flex style="padding:0px;">
                                                <Gauge v-bind:data="gaugeData['load']"/>
                                            </v-flex>
                                        </v-layout>
                                    </v-card-actions>
                                </v-card>
                            ...
                            <v-flex>
                                <FaultsWidget v-bind:navigation="true"/>
                            </v-flex>
                        </v-layout>
                    </v-flex>
                </v-layout>
...
<script>
    ...
    export default {
        ...
        methods: {
            average: function (param, count, data, timestamp) {
                ...
                for (let i = 1; i <= count; i++) {
                    if (data[timestamp][param + i] != -1000)
                        sum += data[timestamp][param + i];
                    else
                        count--;
                }
            ...
            setData() {
                ...
                if (globalStore.type != 'last' && globalStore.type != 'today' && temp.length)
                    this.vesselData['timeStamp'].value = new Date(temp[temp.length - 2]).toISOString().slice(0, 10);

                let params = ['LAT', 'LON'];

                if (typeof globalStore.signals[last] != 'undefined') {
                ...
                if (typeof globalStore.signals[timestamp] != "undefined") {
                    for (let p = 0; p < params.length; p++) {
                    ...
                    if (typeof globalStore.signals[timestamp]['load'] != 'undefined' && this.vesselData['ME_Power_perc'].value == -1000 && globalStore.signals[timestamp]['load'] != -1000)
                ...
                for (let i = 0; i < timeStamps.length; i++) {
                    if (globalStore.signals[timeStamps[i]]['LON'] != -1000) {
                        this.vesselData.route.push({'coords': [globalStore.signals[timeStamps[i]]['LON'], globalStore.signals[timeStamps[i]]['LAT']]});
                    }
                }

                this.$set(this.vesselData, 'points', []);

                if (typeof globalStore.signals[last] != 'undefined' && globalStore.signals[last]['LAT'] != -1000) {
                ...
                if (typeof globalStore.reference[timestamp] != 'undefined') {
                    this.$set(this.engineData, 'engineColor', globalStore.reference[timestamp]['eng_KPI_color']);

                    kpi = globalStore.reference[timestamp]['eng_KPI'] * 100;
                }
                ...
                for (let i=0;i<intNum;i++) {
                    ...
                    let elCount = [this.cylCount, this.cylCount, this.tcCount, this.tcCount, 0, 0];
        ...
        computed: {
            ...
            selVessel: function () {
                return globalStore.selectedVessel;
            },
            cylCount: function () {
